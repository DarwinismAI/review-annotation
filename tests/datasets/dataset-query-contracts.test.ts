...
assert.match(dashboardRoute, /const queryDatasetTotals = async \(\)/);
assert.match(dashboardRoute, /const queryActiveAnnotators = async \(\)/);
assert.match(dashboardRoute, /const queryRecentDatasets = async \(\)/);
assert.doesNotMatch(dashboardRoute, /const queryGlobalTotals = async \(\)/);
assert.doesNotMatch(dashboardRoute, /const queryDashboard = async \(\)/);
assert.match(dashboardRoute, /context\.timing\.measure\("sql",\s*\(\) =>\s*Promise\.all\(\[\s*queryDatasetTotals\(\),\s*queryActiveAnnotators\(\),\s*queryRecentDatasets\(\),\s*\]\),?\s*\)/);
assert.equal((dashboardRoute.match(/await db\.execute\(sql`/g) ?? []).length, 3);
