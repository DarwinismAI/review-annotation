...
export default function AdminAnnotatorsPage() {
  ...
  useEffect(() => {
    fetch("/api/annotators")
      .then((r) => r.json())
      .then((json) => {
        if (json.data) setAnnotators(json.data);
      })
      .catch(() => {})
  ...
  async function updateExpertStatus(expert: Expert, status: "active" | "inactive") {
    ...
    try {
      const res = await fetch(`/api/annotators/${expert.id}`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        setStatusError(json.error?.message ?? "Không cập nhật được trạng thái annotator");
        return;
      }
      setAnnotators((prev) =>
        prev.map((item) =>
  ...
  return (
    <>
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900">Danh sách annotator</h1>
        </div>
