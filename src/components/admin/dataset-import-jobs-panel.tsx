...
export function DatasetImportJobsPanel({ datasetId }: DatasetImportJobsPanelProps) {
  const resourceUrl = `/api/datasets/${datasetId}/import-jobs?pageSize=5`;
  const { data, error, isInitialLoading, isRefreshing, reload } = useFastResource<ImportJobsPayload>(resourceUrl, EMPTY_JOBS);
  ...
  async function runAction(jobId: string, action: "retry" | "cancel") {
    ...
    try {
      ...
      invalidateFastResource(`/api/datasets/${datasetId}/import-jobs`);
      reload();
    } catch {
  ...
  return (
    <section className="space-y-3 rounded-md border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button type="button" size="sm" variant="outline" onClick={reload} disabled={isRefreshing}>
      ...
      {!isInitialLoading && jobs.length === 0 && <div className="text-sm text-slate-500">Chưa có import job.</div>}
      ...
      {isRefreshing && jobs.length > 0 ? <div className="text-xs text-slate-400">Đang cập nhật</div> : null}
    </section>
  );
}
